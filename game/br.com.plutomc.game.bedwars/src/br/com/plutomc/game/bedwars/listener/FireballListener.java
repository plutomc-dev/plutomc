...
public class FireballListener implements Listener {
      ...
      if (event.getItem() != null) {
         if (event.getAction() == Action.RIGHT_CLICK_BLOCK || event.getAction() == Action.RIGHT_CLICK_AIR) {
            ...
            if (event.getItem().getType() == Material.FIREBALL) {
               ...
               player.setMetadata("no-damage-by-entity", GameAPI.getInstance().createMeta(Integer.valueOf(fireball.getEntityId())));
            }
         }
      }
   }
   ...
   public void onBlockPlace(BlockPlaceEvent event) {
      ...
      if (block.getType() == Material.TNT) {
         ...
         player.setMetadata("no-damage-by-entity", GameAPI.getInstance().createMeta(Integer.valueOf(tntPrimed.getEntityId())));
   ...
   public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
      if ((event.getDamager() instanceof Fireball || event.getDamager() instanceof TNTPrimed) && event.getEntity() instanceof Player) {
         ...
         if (player.hasMetadata("no-damage-by-entity")) {
            ...
            event.setCancelled(true);
            player.removeMetadata("no-damage-by-entity", GameMain.getInstance());
            player.setMetadata("nofall", GameAPI.getInstance().createMeta(Long.valueOf(System.currentTimeMillis() + 3000L)));
         }
      }
   }
}
