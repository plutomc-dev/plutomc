...
public class GameListener implements Listener {
      ...
      if (event.getEntity() instanceof Egg && event.getEntity().getShooter() instanceof Player) {
         ...
         if (island != null) {
            ...
            (new BukkitRunnable() {
               ...
               .runTaskTimer(GameAPI.getInstance(), 5L, 0L);
         }
      }
   }
   ...
   public void onPlayerMoveUpdate(PlayerMoveUpdateEvent event) {
      ...
      if (player.hasMetadata("player-target")) {
         if (gamer.isAlive()) {
            ...
            player.removeMetadata("player-target", GameAPI.getInstance());
         }
      }
   }
   ...
   public void onPlayerDeath(PlayerDeathEvent event) {
      ...
      if (!gamer.isSpectator()) {
         if (gamer.isAlive()) {
            ...
            player.removeMetadata("player-armor", GameAPI.getInstance());
            if (finalKill) {
               this.spectate(player);
            } else {
               this.respawn(player);
      ...
      if (event.getType() == UpdateEvent.UpdateType.SECOND) {
         for(Entry<UUID, Long> next : ImmutableList.copyOf(this.playerJoinMap.entrySet())) {
            if (next.getValue() + 45000L < System.currentTimeMillis()) {
               ...
               Gamer gamer = GameAPI.getInstance().getGamerManager().getGamer(next.getKey(), Gamer.class);
               gamer.setAlive(false);
               gamer.setSpectator(true);
               ...
               if (island != null) {
                  if (island.getIslandStatus() == Island.IslandStatus.ALIVE
                     && island.getTeam()
                           .getPlayerSet()
                           .stream()
                           .map(id -> GameAPI.getInstance().getGamerManager().getGamer(id, Gamer.class))
                           .filter(g -> g.isAlive())
                           .count()
                        == 0L) {
   ...
   private void respawn(final Player player) {
      ...
      player.setMetadata("invencibility", GameAPI.getInstance().createMeta(Long.valueOf(System.currentTimeMillis() + 5000L)));
