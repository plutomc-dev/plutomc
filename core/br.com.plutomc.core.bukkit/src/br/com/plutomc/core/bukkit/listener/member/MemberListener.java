...
public class MemberListener implements Listener {
   ...
   public void onPlayerLoginLW(PlayerLoginEvent event) {
      if (CommonPlugin.getInstance().getMemberManager().getMember(event.getPlayer().getUniqueId()) == null) {
         event.disallow(org.bukkit.event.player.PlayerLoginEvent.Result.KICK_OTHER, CommonPlugin.getInstance().getPluginInfo().translate("account-not-loaded"));
      } else {
   ...
   public void onPlayerLogin(final PlayerLoginEvent event) {
      if (CommonPlugin.getInstance().getMemberManager().getMember(event.getPlayer().getUniqueId()) == null) {
         ...
         } else if ((event.getResult() == org.bukkit.event.player.PlayerLoginEvent.Result.KICK_WHITELIST || !CommonPlugin.getInstance().isJoinEnabled())
