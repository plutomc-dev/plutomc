            ...
            case "create":
               ...
               BukkitCommon.getInstance()
                  ...
                  .loadChat(
                     ...
                     confirm,
                     validator,
                     "§aPara criar uma medalha, digite qual será a cor usada\n§aPaleta de cores disponíveis: "
                        + Joiner.on(' ')
                           .join(Arrays.asList(ChatColor.values()).stream().map(chatColor -> "" + chatColor + chatColor.getName()).collect(Collectors.toList())),
                     "§aInsira qual será o simbolo da medalha:",
                     "§aInsira quais serão as aliases da medalha, utilizando vírgula para separar"
                  );
               break;
            default:
               ...
               switch(var14) {
                  case "add":
                     ...
                     if (member.addMedal(medal1)) {
                        sender.sendMessage("§aVocê deu a medalha " + medal1.getChatColor() + medal1.getMedalName() + "§a para o " + member.getPlayerName() + ".");
                     } else {
                        sender.sendMessage(
