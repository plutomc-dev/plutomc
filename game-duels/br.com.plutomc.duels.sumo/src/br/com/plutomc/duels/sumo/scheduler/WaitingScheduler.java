package br.com.plutomc.duels.sumo.scheduler;
...
import br.com.plutomc.duels.sumo.GameMain;
...
public class WaitingScheduler implements Scheduler, Listener {
    private static final ActionItemStack ACTION_ITEM_STACK = new ActionItemStack(
        ...
        public boolean onInteract(Player player, Entity entity, Block block, ItemStack item, ActionItemStack.ActionType action) {
            GameAPI.getInstance().sendPlayerToServer(player, new ServerType[]{CommonPlugin.getInstance().getServerType().getServerLobby(), ServerType.LOBBY});
            return false;
        }
    }
    );
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        ...
        if (player.hasPermission("command.admin") && member.getMemberConfiguration().isAdminOnJoin()) {
            player.setMetadata("admin", new FixedMetadataValue(GameAPI.getInstance(), true));
        } else {
    ...
    public void onPlayerQuit(final PlayerQuitEvent event) {
        ...
        if (event.getPlayer().hasMetadata("admin")) {
            event.getPlayer().removeMetadata("admin", GameAPI.getInstance());
        }
    }
    ...
    public void onPlayerAdmin(PlayerAdminEvent event) {
        ...
        if (player.hasMetadata("admin")) {
            player.removeMetadata("admin", GameAPI.getInstance());
    ...
    public void pulse() {
        ...
        if (GameAPI.getInstance().isConsoleControl()) {
            ...
            if (GameAPI.getInstance().isTimer()) {
                if (!this.timeWasReduced && alivePlayers == 2 && time > 15) {
        ...
        if (time > 0 && (time <= 5 || time % 30 == 0 || time == 15)) {
            Bukkit.getOnlinePlayers().forEach(player -> {
                ...
                player.sendMessage(Language.getLanguage(player.getUniqueId()).t("sumo-game-will-start", "%time%", s + time));
    ...
    private void broadcast(String tag, String playerName, boolean leave) {
        if (leave) {
            Bukkit.broadcastMessage(
                  playerName + " §esaiu na sala (§b" + GameMain.getInstance().getAlivePlayers().size() + "§e/§b" + Bukkit.getMaxPlayers() + "§e)"
            );
        } else {
            Bukkit.broadcastMessage(
                    playerName + " §eentrou na sala (§b" + GameMain.getInstance().getAlivePlayers().size() + "§e/§b" + Bukkit.getMaxPlayers() + "§e)"
            );
        }
    }
}